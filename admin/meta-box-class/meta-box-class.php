...
class AT_Meta_Box {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_stylesheet_directory_uri() . '/admin/meta-box-class';
      }elseif($meta_box['use_with_theme'] === false){
        $this->SelfPath = plugins_url( '/admin/meta-box-class', plugin_basename( dirname( __FILE__ ) ) );
      }else{
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/remove.png' alt='" . __( 'Delete' ) . "' /></a>";
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           ...
           echo '<div class="at-repater-block">'.$mmm;
           echo ' <span class="at-re-toggle"><img src="';
           ...
           echo '<br/><table class="repeater-table" style="display: none;">';
           if ($field['inline']){
        ...
        echo '" alt="'.__('Remove').'" title="'.__('Remove').'" id="remove-'.$field['id'].'"></div><div class="repater-mover"></div>';
        $c = $c + 1;

        }
      }

    echo '<span id="add-'.$field['id'].'" class="button-secondary add-newbutton">'.__('Add New').'</span>';
    echo '</div><br/></div>';
